...
exports.addLeader = async (req, res, next) => {
  try {
    ...
    if (loggedInUser.admin) {
      ...
      if (user.leader) {
        const error = new Error("This user is already a President.");
        error.statusCode = 400;
        throw error;
      }
      ...
      if (oldLeader) {
        ...
        return res.status(201).json({
          message: "President updated old President successfully!",
          data: oldLeader,
        });
      }
  ...
  try {
    ...
    if (loggedInUser.admin) {
      ...
      if (!group.leader) {
        ...
        if (!user.leader) {
          const error = new Error("This user isn't a President.");
          error.statusCode = 400;
          throw error;
        }
...
exports.lockLeader = async (req, res, next) => {
  try {
    ...
    if (loggedInUser.admin) {
      ...
      return res.status(201).json({
        message: "Leader locked successfully!",
        data: leader,
      });
    } else {
      const error = new Error("Not authorized!");
...
exports.unlockLeader = async (req, res, next) => {
  try {
    ...
    if (loggedInUser.admin) {
      ...
      return res.status(201).json({
        message: "Leader unlocked successfully!",
        data: leader,
      });
    } else {
      const error = new Error("Not authorized!");
